...
public class BasicAuthenticationFilterSkeleton implements ContainerRequestFilter {
	...
	public void filter (final ContainerRequestContext requestContext) throws NullPointerException, BadRequestException, NotAuthorizedException {
		...
		//   the credential's password either programmatically, or using HashTools.sha256HashCode(). Convert
		//   the result into a hex-string representation (i.e. 2 digits per byte).
		// - if this hash representation is equal to queried person's password hash,
